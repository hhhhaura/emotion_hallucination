...
from emotion.utils.context_retrieval import grade_squad_samples, load_squad_examples
...
def main() -> None:
    ...
    examples_all = load_squad_examples(split=cfg.split, limit=cfg.limit_problems)
    examples_all = prepare_examples(examples_all, cfg, dataset_name="squad_v2")
    ...
    log.info(
        "[squad_v2] problems=%s skipped=%s N=%s max_new_tokens=%s split=%s stimulus=%s output=%s",
        len(examples),
    ...
    for idx, ex in enumerate(tqdm_wrap(examples, total=len(examples), desc="squad_v2")):
        outputs = lm.sample([ex["messages"]] * cfg.N, max_new_tokens=cfg.max_new_tokens)
        samples, metrics = grade_squad_samples(outputs, ex["reference"])
        ...
        log.info(
            "[squad_v2] %s/%s em=%.2f f1=%.2f wrote=%s",
            idx + 1,
            len(examples),
            metrics["em"],
            metrics["f1"],
