...
def main() -> None:
    ...
    examples_all = prepare_examples(examples_all, cfg, dataset_name="math_en_aligned")
    ...
    log.info(
        "[math_en_aligned] problems=%s skipped=%s N=%s max_new_tokens=%s split=%s stimulus=%s output=%s",
        len(examples),
    ...
    for idx, ex in enumerate(tqdm_wrap(examples, total=len(examples), desc="math_en_aligned")):
        ...
        log.info(
            "[math_en_aligned] %s/%s acc=%.3f wrote=%s",
            idx + 1,
            len(examples),
            metrics["accuracy"],
            wrote_path,
